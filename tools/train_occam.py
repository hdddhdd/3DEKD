...
def main():
    ...
    # distill_cfg 설정 구성
    distill_cfg = None
    if cfg.get('KD', {}).get('ENABLED', False):
        attr_map_dir = cfg.KD.ATTR_MAP_DIR  
        pcl_dir = cfg.KD.PCL_DIR  
        lambda_distill = cfg.KD.LAMBDA_DISTILL
        ply_save_dir = cfg.KD.PLY_SAVE_DIR

        distill_cfg = {
            'ENABLED': True,
            'ATTR_MAP_DIR': attr_map_dir,  # 어트리뷰션 맵 폴더 경로를 설정합니다.
            'LAMBDA_DISTILL': lambda_distill,
            'PCL_DIR': pcl_dir,
            'PLY_SAVE_DIR': ply_save_dir
            
        }
    occam_config = None
    if cfg.get('OCCAM', {}).get('ENABLED', False):
        occam_config = cfg.OCCAM
        # print('*'*100)
    ...
    train_model(
        ...
        optimizer,
        train_loader,
        model_func=model_fn_decorator(distill_cfg=distill_cfg), #, occam_config=occam_config), #distill_cfg 함께 전달
